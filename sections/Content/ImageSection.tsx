  ...
  return (
    <div class="w-full">
      ...
      >
        <Image
          width={640}
          class="w-full lg:w-1/2 object-fit z-10 rounded"
        ...
        <div class="w-full lg:w-1/2 space-y-2 lg:space-y-4 lg:max-w-xl gap-4 z-10">
          <p class="text-[40px] leading-[110%] font-semibold text-[#40E0D0]">
            {title}
          </p>
          <p class="text-black text-[16px] md:text-[18px] leading-[150%]">
            {description}
          </p>
          {cta?.href && cta?.text && (
            <a
              class="w-fit px-3 py-2 flex gap-2 rounded bg-[#FFA500] text-white transition-colors duration-200 cursor-pointer"
              href={cta.href}
            >
              <span>{cta.text}</span>
              <Icon
